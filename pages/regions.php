<?php 
    ...
    try{
        $liste_regions=$connexion_bd->query("SELECT * FROM region ");
    }
    catch(Exception $e){
        print_r($e);
    }
...
<head>
	<!-- Basic Page Info -->
	<meta charset="utf-8">
	<title>Listes des régions</title>
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            ...
                            <nav aria-label="breadcrumb" role="navigation">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="index.html">Régions</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Listes des régions</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix mb-20">
                        <div class="pull-right">
                            <a href="ajout_region.php" class="btn btn-primary btn-sm scroll-click">Ajouter</a>
                        </div>
                    </div>
                    <table class="table">
                        <thead>
                        ...
                        <?php 
                            $ordre=0;
                            while($region=$liste_regions->fetch()){
                                $ordre+=1;  
                            ?>
                            <tr>
                                <th scope="row"><?php echo $ordre;?></th>
                                <td><?php echo $region['code']?></td>
                                <td><?php echo $region['libelle']?></td>
                                <?php 
                                    if ($region['statut'] == true) 
                                    echo '<td><span class="badge badge-primary">Actif</span></td>'
                                ?>
                                <?php 
                                    if ($region['statut'] == false) 
                                    echo '<td><span class="badge badge-warning">Inactif</span></td>'
                                ?>
                                <td>
                                    <a href="modification_region.php?id=<?=$region['id']?>" class="btn btn-warning btn-sm scroll-click"><i class="icon-copy dw dw-edit-1"></i></a>
                                    <a href="../models/suppression_region_bd.php?id=<?=$region['id']?>" data-toggle="modal" data-target="#Medium-modal" type="button" class="btn btn-danger btn-sm scroll-click"><i class="icon-copy dw dw-trash"></i></a>
                                    <div class="modal fade" id="Medium-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title" id="myLargeModalLabel">Suppression d'une région</h4>
                                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                                </div>
                                                <div class="modal-body">
                                                    <p>Cette action est irréversible</p>
                                                    <p>Vous êtes de vouloir supprimer cette région</p>
                                                </div>
