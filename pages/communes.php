...
<head>
	<!-- Basic Page Info -->
	<meta charset="utf-8">
	<title>Listes des communes</title>
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            <nav aria-label="breadcrumb" role="navigation">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Listes des communes</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix mb-20">
                        <div class="pull-right">
                            <a href="ajout_commune.php" class="btn btn-primary btn-sm scroll-click">Ajouter</a>
                        </div>
                    </div>
                    <table class="table">
                        <thead>
                        ...
                        <?php 
                            $ordre=0;
                            while($commune=$liste_communes->fetch()){
                                $ordre+=1;  
                            ?>
                            <tr>
                                <th scope="row"><?php echo $ordre;?></th>
                                <td><?php echo $commune['code']?></td>
                                <td><?php echo $commune['libelle']?></td>
                                <?php 
                                    if ($commune['statut'] == true) 
                                    echo '<td><span class="badge badge-primary">Actif</span></td>'
                                ?>
                                <?php 
                                    if ($commune['statut'] == false) 
                                    echo '<td><span class="badge badge-warning">Inactif</span></td>'
                                ?>
                                <td><?php 
                                    $id_prefecture = $commune['prefecture_id'];
                                    
                                    $prefecture = $connexion_bd->prepare('SELECT *  FROM prefecture WHERE id=:id');
                                    $prefecture->execute([':id'=> $id_prefecture]);
                                    $reponse = $prefecture->fetch(PDO::FETCH_OBJ);
                                    echo $reponse->libelle;
                                ?></td>
                                <td>
                                    <a href="modification_commune.php?id=<?=$commune['id']?>" class="btn btn-warning btn-sm scroll-click"><i class="icon-copy dw dw-edit-1"></i></a>
                                    <a href="../models/suppression_commune_bd.php?id=<?=$commune['id']?>" data-toggle="modal" data-target="#Medium-modal" type="button" class="btn btn-danger btn-sm scroll-click"><i class="icon-copy dw dw-trash"></i></a>
                                    <div class="modal fade" id="Medium-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title" id="myLargeModalLabel">Suppression d'une commune</h4>
                                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                                </div>
                                                <div class="modal-body">
                                                    <p>Cette action est irréversible</p>
                                                    <p>Vous êtes de vouloir supprimer cette commune</p>
                                                </div>
